...
> See `MEMORY-INSTRUCTIONS.md` for full rules. Summary:

1. Read `hotcache.md` first — if answer is fully there, respond directly
2. Read `memory/active.md`, `memory/summary.md`, `memory/decisions.md` **in parallel**
3. If `memory/` doesn't exist → create it from templates
4. Briefly confirm context to the user (1-2 lines)
5. After every task → auto-save memory + rewrite hotcache (never ask)

### Hotcache Rules
- Hard limit: 500 words — overwrite every session end, never append
- Sections: **Active Topics** | **Recent Operations** | **Pending** | **Key Facts**
- Drop context older than ~3 sessions
